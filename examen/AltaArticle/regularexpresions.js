...
function carregarPoblacio() {
  addOptions("poblacions", ["Cantabria", "Asturias", "Galicia", "Andalucia", "Extremadura","olot","figueres"].sort());
}
...
function addOptions2(domElement, array) {
  for (value in array) {
    let option = document.createElement("option");
    option.text = array[value];
    document.getElementsByName(domElement)[1].add(option);
  }
}
function Comprovar() {
  document.getElementById('codi').addEventListener("change", ComprovarCodi);
  document.getElementById('edifici').addEventListener("change", ComprovarEdifici);
  
  
  ComprovarMides();
  ComprovarUbicacions();
  document.getElementsByName('provincia')[0].addEventListener("change", ComprovarCodi);
  document.getElementsByName('poblacions')[0].addEventListener("change", ComprovarEdifici);
...
function ComprovarEdifici() {
  ...
  array = document.getElementsByTagName("select")[1].value.slice(0, 5).split("");
  console.log(array);
  regex = new RegExp("^["+array[0].toUpperCase()+array[0].toLowerCase()+"]["+array[1].toUpperCase()+array[1].toLowerCase()+"]["+array[2].toUpperCase()+array[2].toLowerCase()+ "]["+array[3].toUpperCase()+array[3].toLowerCase()+ "]["+array[4].toUpperCase()+array[4].toLowerCase()+ "]\\+\\d{3}\\.[a-z]{2}$");
  canviarImatge(4, regex.test(document.getElementById('edifici').value));
}
