...
export default class Factura {
    ...
    PintarFactura() {
        ...
        this.Articles.forEach((article, index) => {
            tbodyEl.insertAdjacentHTML("beforeend", ` <tr> <td>${article.Codi}</td><td>${article.Nom}</td><td><input type="number" name= quantitat${index}  value="${article.Quantitat}"></td><td id = preu${index}>${article.Preu}</td><td id = total${index}>${article.Total}</td><td id = aumentar${this.Articles.length}><button id="button${index}" type="button">Sumar</button></td><td id = restar${index}><button id="button${index}type="button">restar</button></td></tr>`);
            document.getElementsByName('quantitat' + index)[0].addEventListener("change", CalcularTotalArticle(index, article, this));
            document.getElementById('aumentar' + index).addEventListener("click", aumentar(article,this));
            document.getElementById('restar' + index).addEventListener("click", restar(article,this));
        });
    }
    PintarArticleFactura(article) {
        ...
        tbodyEl.insertAdjacentHTML("beforeend", ` <tr> <td>${article.Codi}</td><td>${article.Nom}</td><td><input type="number" name= quantitat${this.Articles.length}  value="${article.Quantitat}"></td><td id = preu${this.Articles.length}>${article.Preu}</td><td id = total${this.Articles.length}>${Total}</td><td id = aumentar${this.Articles.length}><button id="button${this.Articles.length}" type="button">Sumar</button></td><td id = restar${this.Articles.length}><button id="button${this.Articles.length}type="button">restar</button></td></tr>`);
        ...
        document.getElementById('aumentar' + this.Articles.length).addEventListener("click", aumentar(this.Articles[this.Articles.length - 1]),this);
        document.getElementById('restar' + this.Articles.length).addEventListener("click", restar(this.Articles[this.Articles.length - 1],this));
    }
...
function aumentar(article,factura){
    return function () {
        article.AumentarQuantitat(); 
        console.log(article);
    }
}
function restar(article,factura){
    return function () {
        article.RestarQuantitat(); 
        console.log(article);
    }
}